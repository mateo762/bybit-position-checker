...
def check_and_close_position(position, current_price, mongo, bybit):
    ...
    if position['status'] == "OPEN":
        ...
        would_close, change_status, closed_tp3 = check_would_close(current_price, position_type, threshold_low,
                                                                   threshold_high,
                                                                   close="TP3")
        if would_close:
            if closed_tp3:
    ...
    if would_close:
        ...
        if position_type == "Short":
            ...
            time.sleep(1)
            bybit.close_position(position, position['accountNumber'], position['lastTransactionNumber'] + 1,
                           position['status'])
            time.sleep(1)
